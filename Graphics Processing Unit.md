...
**Bus type:** 决定显存频率，一般有DDR2、DDR3、GDDR3、GDDR5等几个类型, 据悉GDDR6显存总共能够提供14Gbps的显存速度，而目前最快的GDDR5X则是11Gbps. (1Gbps: 每秒1兆bit,数据传输速度).
...
**Bandwidth:** 显寸带宽：显存带宽是指显示芯片与显存之间的数据传输速率，它以字节/秒为单位。显存带宽是决定显卡性能和速度最重要的因素之一。
